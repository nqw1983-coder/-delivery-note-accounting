...
- **2026-06-09(验证 + 缓存才是元凶)**: 用户报"改的金额没显示(万醉5月显示 36659.6 而非 5719)""空白行不能改"。云端 `payment_state` 查到覆盖值确实在(`pe:2026:万醉:5=5719` 等);**全新浏览器开线上 `0609-2037` 实测:万醉5月正确显示 5719、空白行店名/金额/付款全部可改(空白测试X → 555🌹)**。⇒ **代码无 bug,根因是用户设备跑旧缓存(iOS PWA 缓存顽固,重开不够)**。处置:让用户无痕窗口验证(绕缓存) + 删主屏图标重装 / 清 Safari 数据,使两台都到 `0609-2037`。**再次印证头号血泪:用户报"没改过来",先确认版本号/缓存,别改代码。**
